...
# bach = To access the docker container on the command line
bash:
	$(DockerRun) $(MountProject) --rm client bash

# ci = run lint, type-check and test
ci:
...
# run-dev = run client on a webpack dev server
...
# run = run client from a static build
...
# build = build project and copy output to host machine
...
.PHONY: bash ci test run-dev run build
